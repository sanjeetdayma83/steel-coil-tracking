...
function Replace-Method {
  ...
  $rx = '(?s)' + $StartPattern + '.*?(?=\r?\n' + $EndPattern + ')'
  $new = [regex]::Replace($text, $rx, $Replacement, 1)
  if ($new -eq $text) { throw "Patch target not found in $Path : $StartPattern" }
...
Replace-Method $check '^  Future<void> _searchCoils\(String query, int token\) async \{' '^  void _selectSearchResult' $searchMethod.TrimEnd()
...
$scanText = Get-Content $scan -Raw
$cameraBlock = '(?s)(Padding\(\s*padding:\s*const EdgeInsets\.(?:fromLTRB\(16, 0, 16, 16\)|only\(left: 16,\s*right: 16,\s*bottom: 16\)),\s*child:\s*)Row\(\s*children:\s*\[\s*)'
if ([regex]::IsMatch($scanText, $cameraBlock)) {
  $scanText = [regex]::Replace($scanText, $cameraBlock, '$1Wrap(\n                    spacing: 10,\n                    runSpacing: 10,\n                    children: [', 1)
  # Remove Expanded only in the camera controls region, up to the next
  # closing Padding. This avoids touching the rest of the screen layout.
  $match = [regex]::Match($scanText, '(?s)Wrap\(\s*spacing: 10,\s*runSpacing: 10,\s*children: \[.*?\n\s*\],\s*\),', [System.Text.RegularExpressions.RegexOptions]::Singleline)
  if ($match.Success) {
    $block = $match.Value
    $block = $block -replace 'Expanded\(\s*child:\s*_CameraButton\(', '_CameraButton('
    $block = $block -replace '\)\s*\),\s*const SizedBox\(width: 10\),', '),'
    $scanText = $scanText.Substring(0, $match.Index) + $block + $scanText.Substring($match.Index + $match.Length)
  }
}
Set-Content $scan -Value $scanText -Encoding UTF8
